...
class Resumable(object):
    ...
    def prepare(self, work_dir, in_filename, url_chunk_num, url_upload_id, url_group, owner):
        ...
        if chunk_num == 'end':
            ...
            assert Resumable._db_insert_new_for_owner(res_db, upload_id, url_group)
            chunk_order_correct = True
    ...
    def _find_relevant_resumable_dir(self, work_dir, filename, upload_id, res_db):
        ...
        """
        relevant = None
        potential_resumables = Resumable._db_get_all_resumable_ids_for_owner(res_db)
        if not upload_id:
            ...
            for cand in candidates:
                upload_id = cand[1]
                first_chunk = Resumable._find_nth_chunk(work_dir, upload_id, filename, 1)
            ...
            if _IS_VALID_UUID.match(pr):
                try:
                    chunk_size, max_chunk, md5sum, \
                        ...
                        filename = Resumable._get_resumable_chunk_info(current_pr, work_dir, res_db=res_db)
                    if recommendation == 'end':
                        next_offset = 'end'
                except (OSError, Exception):
                    pass
                if chunk_size:
                    group = Resumable._db_get_group(res_db, pr)
    ...
    def _get_resumable_chunk_info(self, resumable_dir, work_dir, res_db=None):
        ...
        def info(chunks, recommendation=None, warning=None):
            ...
            next_offset = Resumable._db_get_total_size(res_db, upload_id)
    ...
    def info(self, work_dir, filename, upload_id, owner):
        ...
        chunk_size, max_chunk, md5sum, \
            previous_offset, next_offset, \
            warning, recommendation, filename = Resumable._get_resumable_chunk_info(resumable_dir, work_dir, res_db=res_db)
        group = Resumable._db_get_group(res_db, upload_id)
        if recommendation == 'end':
            next_offset = 'end'
