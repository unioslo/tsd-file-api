...
def set_config() -> None:
    ...
    if "accept_calls_per_event_loop" in _config:
        tornado.netutil.ACCEPT_CALLS_PER_EVENT_LOOP = _config[
...
class AuthRequestHandler(RequestHandler):
        ...
        try:
            ...
            if not auth_header:
                raise ClientAuthorizationError(
                    "Authorization not possible: missing header"
                )
    ...
    def enforce_group_logic(
        ...
        if group_config["valid_group_regex"]:
            is_valid_groupname = re.compile(
                r"{}".format(group_config["valid_group_regex"])
            )
        ...
        if group_config["ensure_tenant_in_group_name"]:
            if tenant not in group_name:
                raise ClientGroupAccessError(
                    f"tenant: {tenant} not in group name: {group_name}"
                )
...
class SnsFormDataHandler(AuthRequestHandler):
    ...
    def initialize(self, backend: str) -> None:
        ...
        self.group_config = current_backend.get(
            "group_logic",
            {
                "enabled": False,
                "default_url_group": "",
    ...
    def prepare(self) -> Optional[Awaitable[None]]:
        ...
        try:
            ...
            if content_type == "application/octet-stream+nacl":
                ...
                if (
                    int(self.request.headers["Nacl-Chunksize"])
                    > options.max_nacl_chunksize
                ):
            ...
            group_name, group_memberships = self.get_group_info(
                tenant, self.group_config, self.authnz
            )
            self.enforce_group_logic(
                group_name, group_memberships, tenant, self.group_config
            )
            ...
            if self.group_config["enabled"] and self.request.method in ["PUT", "PATCH"]:
                ...
                if (
                    url_unescape(uri_group_name) != self.filename
                    and uri_group_name != group_name
                ):
                    raise ClientGroupAccessError(
            ...
            elif self.group_config["enabled"] and self.request.method in [
                "GET",
                "HEAD",
            ]:
    ...
    def get(self, tenant: str, filename: str = None) -> None:
        ...
        try:
            ...
            if "Range" not in self.request.headers:
                ...
                if "," in start_and_end:
                    raise ClientMethodNotAllowed(
                        "Multipart byte range requests not supported"
                    )
                ...
                if client_end > full_file_size:
                    raise ClientContentRangeError(
                        "Range request exceeds byte range of resource"
                    )
...
class GenericTableHandler(AuthRequestHandler):
    ...
    def decrypt_nacl_data(
        self, data: bytes, headers: tornado.httputil.HTTPHeaders
    ) -> str:
        out = b""
        nacl_stream_buffer = data
        nacl_nonce = options.sealed_box.decrypt(base64.b64decode(headers["Nacl-Nonce"]))
        nacl_key = options.sealed_box.decrypt(base64.b64decode(headers["Nacl-Key"]))
        try:
...
class NaclKeyHander(RequestHandler):
    def get(self, tenant: str) -> None:
        ...
        out = {
            ...
            "exp": None,
            "usage": {
                "explanation": "To be used in combination with encrypted stream "
    ...
    optional_routes = {
        "files_import": [
            (
                "/v1/(.*)/files/stream",
                FileRequestHandler,
                dict(backend="files_import", namespace="files", endpoint="stream"),
            ),
            (
                "/v1/(.*)/files/stream/(.*)",
                FileRequestHandler,
        ...
        "survey": [
            ...
            (
                "/v1/(.*)/survey/([a-zA-Z_0-9*]+)/submissions",
                GenericTableHandler,
                dict(backend="survey"),
            ),
            (
                "/v1/(.*)/survey/([a-zA-Z_0-9*]+/audit)",
                GenericTableHandler,
                dict(backend="survey"),
            ),
            (
                "/v1/(.*)/survey/([a-zA-Z_0-9]+)$",
                GenericTableHandler,
                dict(backend="survey"),
            ),
        ...
        "publication": [
            (
                "/v1/(.*)/publication/import",
                FileRequestHandler,
                dict(backend="publication", namespace="publication", endpoint="import"),
            ),
            (
                "/v1/(.*)/publication/import/(.*)",
                FileRequestHandler,
            ...
            (
                "/v1/(.*)/publication/resumables",
                ResumablesHandler,
                dict(backend="publication"),
            ),
            (
                "/v1/(.*)/publication/resumables/(.*)",
                ResumablesHandler,
            ...
            (
                "/v1/(.*)/publication/export",
                FileRequestHandler,
                dict(backend="publication", namespace="publication", endpoint="export"),
            ),
            (
                "/v1/(.*)/publication/export/(.*)",
                FileRequestHandler,
        ...
        "apps_files": [
            ...
            (
                "/v1/(.*)/apps/(.+/files.*)",
                FileRequestHandler,
                dict(backend="apps_files", namespace="apps", endpoint=None),
            ),
        ],
        "apps_tables": [
            (
